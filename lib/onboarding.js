...
export async function probeSpeakers({ account = {}, store = null, did, timeoutMs = 15000 } = {}) {
  ...
  //
  // ⚠️ ssecurity 位于 store.pass.ssecurity，**顶层没有这个字段**。
  // 写成 store?.ssecurity 会恒为 undefined，导致明明有可用 token 却被判定
  // "两者都没有"，或退化成账号密码登录并触发异地登录风控。
  // （对照本文件 194 行的正确写法：node.pass?.ssecurity。）
  const hasToken = Boolean(store?.serviceToken && (store?.pass?.ssecurity || store?.ssecurity));
...
export async function importCredentialsFromHa(opts) {
  ...
  // did/hardware 缺失时，从【已有 store】或【HA 的设备列表】尽力补全。
  // 为什么必须补：vendor 的 getConversations 用 device.hardware 作查询参数，
  // 空值会被小米判 400，插件每 4 秒空转一次（实测踩过 711 次 400）。
  let resolvedDid = did;
  let resolvedHardware = hardware;
  if (!resolvedDid || !resolvedHardware) {
    try {
      const existing = JSON.parse(readFileSync(resolve(storePath), "utf8"));
      const anyNode = existing?.mina ?? existing?.miiot ?? {};
      resolvedDid = resolvedDid || anyNode.did || anyNode.device?.did || "";
      resolvedHardware = resolvedHardware || anyNode.hardware || anyNode.device?.hardware || "";
    } catch {
      /* 没有历史 store，继续用空值 */
    }
  }
  // 仍未拿到型号 → 从 HA 的 xiaomi_miot 设备缓存里找（它记了每台设备的 model）
  if (!resolvedHardware) {
    try {
      const raw = JSON.parse(ssh(`cat ${dir}/devices-${useUid}-cn.json 2>/dev/null`));
      const data = raw?.data ?? raw;
      const list = Array.isArray(data) ? data : (data?.devices ?? []);
      const hit = list.find((d) => String(d.did ?? d.miotDID ?? d.deviceId) === String(resolvedDid))
        ?? list.find((d) => /speaker|soundbox|xiaomi\.mico/i.test(String(d.model ?? "")))
        ?? list[0];
      if (hit) {
        resolvedDid = resolvedDid || String(hit.did ?? hit.miotDID ?? "");
        resolvedHardware = String(hit.model ?? "").toUpperCase();
      }
    } catch {
      /* 设备缓存读不到就维持空值，用户可手填 */
    }
  ...
  const toSegment = (src, sid) => {
    ...
    // ⚠️ hardware / did 必须非空，否则 vendor 的 getConversations 会拿空值
    // 当查询参数 → 小米返回 400，插件每 4 秒一次 400 空转（实测踩过）。
    // HA 的 auth 文件里只有 device_id，因此这两个值必须由调用方提供。
    return {
