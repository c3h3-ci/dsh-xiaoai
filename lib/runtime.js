...
export class XiaoaiRuntime {
  ...
  #resolveIntent(text) {
    ...
    if (this.#aiMode === "idle") {
      ...
      // （若同时配置了 triggerKeywords，也认它，保持老配置可用）
      const direct = this.#matchesAny(raw, this.#config.callAIKeywords)
        || this.#matchesAny(raw, this.#config.triggerKeywords)
        || (this.#config.triggerKeywords.length === 0 && this.#config.callAIKeywords.length === 0);
      return direct ? { action: "ask", text: raw } : { action: "ignore", text: raw };
    }
  ...
  async #advanceAiMode(text) {
    ...
    void this.#sayPhrase(this.#config.onAIAsking, "思考");
    this.#touchKeepAlive();
    return true;
  }
  ...
  async #tick() {
    ...
    for (const rec of fresh) {
      ...
      try {
        ...
        await this.#sayPhrase(this.#config.onAIError, "错误");
