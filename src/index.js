...
function buildSettingsSchema(z) {
  return z.object({
    ...
    speakers: z
      .array(
        z.object({
          ...
          // ── 每设备覆盖（null = 继承全局）──
          // ⚠️ schemastery 没有 .nullable()（调用会抛 "z.string(...).nullable is not a function"，
          // 整个插件装配失败）。用 z.union([T, z.const(null)]) 表达「可为 null」。
          workspace: z.union([z.string(), z.const(null)]).default(null),
          agentPreset: z.union([z.string(), z.const(null)]).default(null),
          provider: z.union([z.string(), z.const(null)]).default(null),
          /** 启动时设定该设备音量（0-100）。 */
          volume: z.union([z.number(), z.const(null)]).default(null),
          /** 覆盖全局唤醒词。空数组 = 这台不要唤醒词（有效覆盖）。 */
          wakeUpKeywords: z.union([z.array(z.string()), z.const(null)]).default(null),
        }),
      )
      .default([]),
    ...
    /** 本地快速路径：音量/时间/停止等高频指令不走 LLM（毫秒级响应）。 */
    localCommandsEnabled: z.boolean().default(DEFAULTS.localCommandsEnabled),
    ...
    /** 长任务进度安抚语（空数组 = 不播报）。 */
    onAIProgress: z.array(z.string()).default([...DEFAULTS.onAIProgress]),
    /** 多久无结果就播进度语（秒，最小 10）。 */
    progressAfterSeconds: z.number().default(DEFAULTS.progressAfterSeconds),
    /** 保留多少轮对话历史（供 UI 回看）。 */
    historyLimit: z.number().default(DEFAULTS.historyLimit),
    /** 出错时的提示语（分类未命中时的兜底）。 */
