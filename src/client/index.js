...
/* ── 并发冲突提示（revision 乐观锁被拒）─────────────────────
   独立于 .xiaoai-error：冲突不是「出错了」，而是「有别的写入先到」，
   语义上更像警告，因此用 warn 色，并允许内嵌一个动作按钮。 */
.xiaoai-conflict {
	margin: 0;
	padding: 8px 12px;
	font-size: 13px;
...
.xiaoai-device-card[data-tone="ok"] {
	border-left: 3px solid var(--dsw-alias-state-success-primary);
}
.xiaoai-device-card[data-tone="warn"] {
	border-left: 3px solid var(--dsw-alias-state-warn-primary);
}
.xiaoai-device-card[data-tone="error"] {
	border-left: 3px solid var(--dsw-alias-state-error-primary);
}
.xiaoai-device-card[data-tone="muted"] {
	border-left: 3px solid var(--dsw-alias-label-quaternary, rgba(128, 128, 128, 0.4));
}
.xiaoai-device-disabled {
	opacity: 0.68;
}
.xiaoai-device-expanded {
...
.xiaoai-device-dot {
	...
	border-radius: 50%;
	flex: 0 0 auto;
	background: var(--dsw-alias-label-quaternary, rgba(128, 128, 128, 0.4));
}
.xiaoai-device-dot[data-tone="ok"] { background: var(--dsw-alias-state-success-primary); }
.xiaoai-device-dot[data-tone="warn"] { background: var(--dsw-alias-state-warn-primary); }
.xiaoai-device-dot[data-tone="error"] { background: var(--dsw-alias-state-error-primary); }
.xiaoai-device-dot[data-tone="muted"] { background: var(--dsw-alias-label-quaternary, rgba(128, 128, 128, 0.45)); }
.xiaoai-device-name {
	font-weight: 500;
	color: var(--dsw-alias-label-primary);
	display: inline-flex;
...
.xiaoai-device-btn:hover:not(:disabled) {
	background: color-mix(in srgb, var(--dsw-alias-label-primary) 8%, transparent);
}
.xiaoai-device-btn:disabled {
	opacity: 0.5;
	cursor: not-allowed;
}
.xiaoai-device-btn:focus-visible {
...
.xiaoai-device-override-summary {
	margin: 0;
	font-size: 12px;
	line-height: 18px;
	color: var(--dsw-alias-label-secondary);
}
.xiaoai-device-config {
	margin-top: 4px;
	padding-top: 8px;
	border-top: 1px dashed var(--dsw-alias-border-secondary, rgba(128, 128, 128, 0.25));
	display: flex;
	flex-direction: column;
	gap: 6px;
}
.xiaoai-device-picker {
	margin: 8px 0;
	padding: 10px 12px;
	border: 1px dashed var(--dsw-alias-border-secondary, rgba(128, 128, 128, 0.3));
	border-radius: 10px;
	display: flex;
	flex-direction: column;
	gap: 6px;
}
.xiaoai-device-candidate {
	display: flex;
	align-items: center;
	gap: 8px;
	padding: 4px 0;
}
.xiaoai-device-candidate-name {
	min-width: 0;
}
.xiaoai-device-candidate .xiaoai-button {
	margin-left: auto;
}
.xiaoai-device-toolbar {
	margin-top: 4px;
}

/* ── ⑦ 状态与日志：设备状态列表 ────────────────────────────── */
.xiaoai-status-devices {
	display: flex;
	flex-direction: column;
	gap: 4px;
	margin-bottom: 10px;
}
.xiaoai-status-device {
	display: flex;
	align-items: center;
	gap: 8px;
	font-size: 13px;
...
.xiaoai-status-device-name {
	color: var(--dsw-alias-label-primary);
}
.xiaoai-status-device-tone {
	font-size: 12px;
	color: var(--dsw-alias-label-dimmed, var(--dsw-alias-label-tertiary));
}
.xiaoai-status-device-bits {
	font-size: 12px;
	margin-left: auto;
	text-align: right;
}
.xiaoai-recovery-text {
	margin: 0;
	font-size: 13px;
	line-height: 19px;
	color: var(--dsw-alias-label-primary);
...
/* ── 错误边界兜底页（2026-09-21 #310 白屏事故后新增）──────────
   目标：崩溃时用户看到「失败 + 原因 + 重试」，而不是一片空白。
   空白会被误判成「插件没装/坏了」，这是那次事故最伤的地方。 */
.xiaoai-crash {
	gap: 12px;
}
.xiaoai-crash-detail {
...
/* ── 错误诊断卡片 ────────────────────────────────────────
   错误原文保留在上方，这里是「发生了什么 + 该点哪里」。
   目的是把「抱歉，出错了」变成用户能自己动手的指引。 */
.xiaoai-diagnosis {
	display: flex;
	flex-direction: column;
	gap: 4px;
...
.xiaoai-diagnosis-title {
	margin: 0;
	font-size: 13px;
	font-weight: 600;
	color: var(--dsw-alias-label-primary);
}
.xiaoai-diagnosis-detail {
	margin: 0;
	font-size: 12px;
	line-height: 17px;
	color: var(--dsw-alias-label-secondary, inherit);
}
.xiaoai-diagnosis-action {
	margin: 2px 0 0;
	font-size: 12px;
	line-height: 17px;
...
`;
		...
		//#region 并发栅栏 / 回环恢复（dsh-im 对比得出的三项改进）

		/** 回环恢复的固定文案（错误抛出与界面提示共用一处）。 */
		const LOOPBACK_RECOVERY_MESSAGE =
			"当前地址与浏览器的本机请求校验不兼容。请使用上方按钮改用 localhost 重新打开。";

		/**
		 * 把焦点还给某个触发器。
		 *
		 * 用 queueMicrotask 而不是直接 `.focus()`：调用点通常在 setState
		 * 之前（或同一批次里），此刻 DOM 还没提交，目标按钮可能已被卸载
		 * 重建 —— 直接 focus 会落到旧节点上，表现为「焦点还是丢了」。
		 * 微任务在本次渲染提交之后运行，拿到的就是新节点。
		 *
		 * @param {{current: (HTMLElement|null)}} ref React ref。
		 */
		 ...
		 * 保存被拒的原因分类。
		 *
		 * 服务端 `xiaoai/settings.update` 用 revision 做乐观锁，冲突时抛
		 * `SettingsConflictError`，被 `src/rpc.js:340-346` 翻译成
		 * code `xiaoai/settings-conflict`。
		 *
		 * ⚠️ 这里判断的是**错误码**而不是文案：错误文案将来可能改，
		 * 码是契约 §4 的一部分。同时也兜住 `revision` 字样，兼容
		 * 老版本服务端只回 message 的情况。
		 */
		function isSettingsConflict(error) {
			if (!error) return false;
			if (error.code === "xiaoai/settings-conflict") return true;
			const message = String(error.message ?? "");
			return /revision|版本.*冲突|settings-conflict/i.test(message) && /冲突|conflict|stale/i.test(message);
		}

		/**
		 * 回环地址恢复。
		 *
		 * 背景：浏览器对 `http://127.0.0.1` 有额外的「本机请求」校验
		 * （PNA / 私网访问策略），某些环境下对同源 fetch 也会回 403。
		 * 这时 dsh-client-connection 会把响应转成
		 *   `transport failure for /api/xiaoai/<method>: HTTP 403`
		 * （见 dsh-client-connection/lib/client.js:1131 的 throw 点）。
		 *
		 * 换成 `localhost` 走的是另一条「可信来源」判定路径，通常能直接恢复，
		 * 所以这里把错误升级成一个【可点击的恢复动作】，而不是干巴巴报错。
		 *
		 * @param {unknown} error 捕获到的异常。
		 * @param {string} href 当前页面地址（默认取 location.href）。
		 * @returns {{url:string, origin:string}|null} 恢复目标；不适用时为 null。
		 */
		function createLoopbackRecovery(error, href) {
			const message = String((error && error.message) || error || "");
			// 只认这一种签名 —— 别的 403（权限不足等）不该被当成地址问题。
			if (!/^transport failure for \/[A-Za-z0-9._~-]+\/[A-Za-z0-9_$./~-]+: HTTP 403$/.test(message)) {
				return null;
			}
			const current =
				typeof href === "string"
			...
			try {
				...
				// 只处理 IPv4 回环，其他主机名保持原样（换成 localhost 可能解析到别处）。
				const parts = url.hostname.split(".");
				const isIpv4Loopback =
					parts.length === 4 &&
					parts[0] === "127" &&
					parts.every((part) => /^\d{1,3}$/.test(part) && Number(part) <= 255);
				if (!isIpv4Loopback) return null;
		 ...
		 * 并发版本栅栏。
		 *
		 * 解决验收报告 ui-acceptance.md「诚实性说明 #3」记录的 P2：
		 * 并发编辑（多标签页 / 手机 + 电脑）时，UI 对 revision 冲突
		 * **没有任何用户可见提示**，表现为「点了保存，但值又变回去了」。
		 *
		 * 栅栏管两件事：
		 *   1. **陈旧响应不覆盖新状态** —— 每次「本地发起修改」递增 version；
		 *      响应回来时若 version 已变，说明中间又发生了新的操作，
		 *      这份响应已经过期，直接丢弃（等价于 dsh-im 的
		 *      `canCommitMutation` / `canCommitStatus`）。
		 *   2. **pending 计数** —— `settings.get` 的响应只有在「没有正在进行的
		 *      保存」时才允许落地，避免慢的读请求把刚保存的值冲掉。
		 *
		 * 用 ref 而不是 state：栅栏本身绝不该触发重渲染（它只是并发记账）。
		 */
		function useSettingsFence() {
			const state = React.useRef({ version: 0, pending: 0 });
			return React.useMemo(
				() =>
					Object.freeze({
						/** 发起一次读：记录版本，响应回来时用 canCommitStatus 校验。 */
						beginStatus() {
							return state.current.pending === 0 ? state.current.version : null;
						},
						/** 这份读响应是否还能落地。 */
		...
		const SETTING_DEFAULTS = {
			...
			// ── 多音箱设备列表（设计 §3.2）──
			//
			// 默认空数组 = 「未使用新结构」→ UI 走单 did 降级路径（向后兼容）。
			// ⚠️ 必须在这里登记，否则 computeDirtyKeys 根本不会比较它，
			// 设备编辑就没有脏点、保存按钮也不亮。
			speakers: []
		};
		...
		function normalizeSettings(values) {
			...
			// ── 多音箱设备列表（特殊：数组元素是**对象**，不是字符串）──
			//
			// ⚠️ 不能放进 LIST_FIELDS：那里会 `String(item)`，把对象变成
			// "[object Object]"，设备信息全丢。这里单独归一化。
			//
			// 同样不能漏掉 —— normalizeSettings 只保留它认识的键，
			// 一个没登记的字段会被**静默丢弃**，表现为「改设备不脏、保存无效」
			// （浏览器实测踩过：脏点不亮、保存按钮不激活）。
			out.speakers = Array.isArray(source.speakers)
				? source.speakers
						.filter((item) => item && typeof item === "object" && !Array.isArray(item))
						.map((item) => ({
							did: item.did === undefined || item.did === null ? "" : String(item.did),
							name: item.name === undefined || item.name === null ? "" : String(item.name),
							model: item.model === undefined || item.model === null ? "" : String(item.model),
							enabled: item.enabled === undefined ? true : Boolean(item.enabled),
							// null = 未覆盖（继承全局）—— 保持 null 语义，不要转成 ""
							workspace: item.workspace === undefined ? null : item.workspace,
							agentPreset: item.agentPreset === undefined ? null : item.agentPreset,
							provider: item.provider === undefined ? null : item.provider
						}))
				: [];
		...
		function buildPatch(draft) {
			...
			//
			// ⚠️ 只在**确实有设备**时才写该字段：
			//   · 老配置（没有 speakers）不该被这次保存凭空写进一个空数组 ——
			//     那会让后端以为用户清空了设备列表。
			//   · 数组元素是对象，不能走 parseList（它按字符串处理）。
			if (Array.isArray(draft.speakers) && draft.speakers.length > 0) {
				patch.speakers = draft.speakers.map((item) => ({
					did: String(item.did || ""),
			...
			for (const key of Object.keys(SETTING_DEFAULTS)) {
				...
				if (Array.isArray(a) || Array.isArray(b)) {
					...
					for (let i = 0; i < left.length; i += 1) {
						// ⚠️ 元素可能是**对象**（如 speakers[] 的 `{did,name,enabled,...}`）。
						// 早期实现用 `String(left[i]) !== String(right[i])`，对对象会
						// 双双得到 "[object Object]" → 任何两台不同设备都「相等」→
						// 改了设备配置却不打脏点、保存按钮也不亮（静默丢改动）。
						// 因此对象元素改走结构化比较。
						const li = left[i];
						const ri = right[i];
						const bothObjects =
							li !== null && typeof li === "object" && ri !== null && typeof ri === "object";
						if (bothObjects) {
							if (!sameRecord(li, ri)) {
								same = false;
								break;
		...
		/**
		 * 两个对象的「配置等价」比较（浅比较，够用且可预测）。
		 *
		 * 只比较**两边都出现过的键**的并集：这样 `{a:1}` 与 `{a:1,b:null}`
		 * 会被判为不同（`b` 显式设成 null 是有效覆盖语义，见设计 §3.2）。
		 *
		 * @param {object} left
		 * @param {object} right
		 * @returns {boolean}
		...
		function sameRecord(left, right) {
			...
			for (const key of keys) {
				...
				if (typeof an === "object" || typeof bn === "object") {
					// 嵌套对象（如 overrides）序列化比较 —— 设备配置层级浅，
					// 键序在这里是稳定的（同一个 normalize 出口产生的）。
		...
		//#region 错误诊断（把原始错误翻译成「发生了什么 + 该点哪里」）
		//
		// 【为什么客户端也要做一份】服务端 `#classifyErrorPhrases`（runtime.js:1217）
		// 只负责**挑一句播报**，它看到的错误已丢失上下文；面板需要的是
		// 「哪一条凭据缺了」「该点哪个按钮」这种**可操作**的指引。
		//
		// ⚠️ 下面的规则全部来自**实测过的日志原文**（2026-09-20 真实日志），
		// 不是凭想象设计的分类。

		/** 诊断规则表；顺序即优先级（先具体后笼统）。 */
		const ERROR_DIAGNOSES = [
			{
				// ⚠️ 必须排在 401 之前：这个文案里含「登录」二字，
				// 会被 401 的规则误捕成「重新登录」，但登录其实是好的 ——
				// 那会把用户引向一个修不好问题的动作。
				id: "missing-iot-credential",
				match: (m) => m.indexOf("登录失败") !== -1 && (m.indexOf(".mi.json") !== -1 || m.indexOf("凭据") !== -1),
				title: "只拿到「拉对话」凭据，缺「控制音箱」凭据",
				detail:
					"小米要两份凭据：一份读语音对话（MiNA），一份让音箱说话（MiIOT）。" +
					"现在只成功了前者 —— 所以能听到你说话，但音箱不回应。",
				action: "点上方「重新接入 / 更换音箱」→ 选「从 HA 导入」，会一次取回两份凭据。"
			},
			{
				id: "risk-control",
				match: (m) => m.indexOf("70016") !== -1 || m.indexOf("验证失败") !== -1 || m.indexOf("notificationurl") !== -1,
				title: "小米检测到异地登录，要求安全验证",
				detail: "这不是密码错，是小米的风控。需要你在浏览器里完成一次授权。",
				action: "重新走一次接入向导，向导会给出授权链接，点开后用小米账号确认即可。"
			},
			{
				id: "hardware-missing",
				match: (m) =>
					(m.indexOf("400") !== -1 && (m.indexOf("获取对话") !== -1 || m.indexOf("hardware") !== -1)) ||
					m.indexOf("hardware") !== -1,
				title: "设备型号信息缺失",
				detail: "导入的凭据里缺 device.hardware，导致每次拉对话都被小米拒绝（表现为 400 反复空转）。",
				action: "点「重新接入 / 更换音箱」重新导入凭据；或在「接入音箱」组里手动补全型号。"
			},
			{
				id: "token-expired",
				match: (m) => m.indexOf("401") !== -1 || m.indexOf("403") !== -1 || m.indexOf("unauthor") !== -1 || m.indexOf("forbidden") !== -1,
				title: "小米账号登录状态已过期",
				detail: "缓存的 token 失效了，且不会自动退回密码登录。",
				action: "回到「接入音箱」组重新保存一次密码；或点「重新接入」→「从 HA 导入」刷新凭据。"
			},
			{
				id: "network",
				match: (m) =>
					/econnrefused|econnreset|enotfound|network|socket|fetch failed|无法连接|网络/.test(m),
				title: "网络连接失败",
				detail: "本机与小米服务器之间网络不通。",
				action: "检查网络与代理设置；若是偶发抖动，稍等会自动恢复。"
			},
			{
				id: "timeout",
				match: (m) => /timeout|aborted|超时|timed out/.test(m),
				title: "请求超时",
				detail: "等不到响应（可能是小米接口慢，或 DSH 侧处理太久）。",
				action: "通常会自动重试；持续出现可适当调大「轮询间隔」。"
			}
		];

		/**
		 * 把原始错误串翻译成「发生了什么 + 该点哪里」。
		 *
		 * @returns {{title,detail,action,raw}|null} 识别不出时返回 null ——
		 *          调用方应保持只显示原文，**不要硬套一个可能错的结论**。
		 */
		function diagnoseError(rawMessage) {
			const text = isBlank(rawMessage) ? "" : String(rawMessage);
			if (text === "") return null;
			const m = text.toLowerCase();
			for (const rule of ERROR_DIAGNOSES) {
				try {
					if (rule.match(m)) {
						return {
							id: rule.id,
		...
		/** 按钮。`options.ref` 用于焦点管理 —— 关闭弹层后把焦点还给触发器。 */
		 ...
		 * 面板错误边界。
		 *
		 * 【为什么必须有】2026-09-21 那次 React #310：`renderSettingsForm` 里
		 * 一个 Hook 归属错误，导致整个 settings.section 白屏 —— 用户看到的是
		 * **一片空白**，以为插件坏了/没装上，实际只是渲染崩了。
		 * 有边界的话，至少是「加载失败 + 具体错误 + 重试」。
		 *
		 * ⚠️ **它是兜底，不是修复**（这点必须说清，别指望它"解决问题"）：
		 *   1. `failed` 一旦置位不会自动恢复，点「重试」只是重置标志，
		 *      同一个 bug 会立刻再崩一次 —— 它让故障**可见**，不让故障**消失**；
		 *   2. 它只能捕获**渲染阶段**的错误。事件处理器 / 异步回调 / 定时器里
		 *      抛出的错误**不会**经过它（React 的设计如此）。
		 *
		 * 【比 dsh-im 版多两点】
		 *   · 显示 error.message —— dsh-im 不显示，用户看不到原因，只能截图问人；
		 *   · 提示去看运行日志 —— 面板崩了但服务端日志还在，那才是排查入口。
		 */
		class PanelErrorBoundary extends React.Component {
			...
			componentDidCatch(error, info) {
				// 控制台留全量堆栈：UI 上只显示一行摘要，细节靠这里。
				console.error("dsh-xiaoai: 设置面板渲染失败", error, info);
			}
			...
			render() {
				...
				return h(
					"section",
					{ className: "xiaoai-section xiaoai-crash", role: "alert" },
					h("h2", { className: "xiaoai-heading" }, "小爱语音"),
					h("p", { className: "xiaoai-error" }, "设置面板加载失败。"),
					this.state.message
						? h("pre", { className: "xiaoai-crash-detail" }, this.state.message)
						: null,
					h(
						"p",
						{ className: "xiaoai-row-hint" },
						"这通常是面板本身的渲染错误，插件仍在后台运行，音箱侧不受影响。" +
							"若反复失败，请在 DSH 日志里搜索「dsh-xiaoai: 设置面板渲染失败」，那里有完整堆栈。"
					),
					h(
						"div",
						...
						h(
							...
							{
								type: "button",
								className: "xiaoai-button xiaoai-button-primary",
		...
		function renderStatusHeader(status) {
			...
			if (safe.phase === "error" && !isBlank(safe.lastError)) {
				const raw = String(safe.lastError);
				const dx = diagnoseError(raw);
				children.push(
					// ① 原文永远保留 —— 诊断可能判错，原文是用户/我们排障的唯一可靠依据。
					h("p", { className: "xiaoai-error", role: "alert", key: "error" }, "错误：" + raw),
					// ② 识别得出时，补一条「发生了什么 + 该点哪里」。
					//    识别不出就**不加** —— 硬套一个可能错的结论比不说更糟。
					dx
						? h(
								"div",
								{ className: "xiaoai-diagnosis", key: "diagnosis" },
								h("p", { className: "xiaoai-diagnosis-title" }, "可能的原因：" + dx.title),
								h("p", { className: "xiaoai-diagnosis-detail" }, dx.detail),
								h("p", { className: "xiaoai-diagnosis-action" }, "👉 " + dx.action)
							)
						: null
				);
			}
		...
		function renderLogs(state) {
			const toggle = h(
				"button",
				{
					...
					key: "toggle",
					// 焦点管理：收起日志后焦点回到这里（见 onToggleLogs）。
					ref: state.toggleRef || null,
					"aria-expanded": state.logsOpen ? "true" : "false",
		 ...
		 */
		//#region 多音箱设备卡片（设计见 docs/research/multi-speaker-design.md §6）

		/**
		 * 归一化「设备列表」。
		 *
		 * ⚠️ 这里是**向后兼容 + 后端未就绪降级**的唯一入口，必须同时满足：
		 *
		 *   1. **老配置**（只有 `did`，没有 `speakers`）→ 合成一台虚拟设备，
		 *      让 UI 有东西可渲染。合成项的 `synthetic: true`，保存时**不写回**
		 *      `speakers` 字段（避免老用户凭空多出一个 speakers 数组）。
		 *   2. **后端未实现 `speakers`**（当前状态）→ 走同一个降级分支，
		 *      界面表现为「单台设备 + 提示」，而不是报错或空白。
		 *   3. **新结构**（`speakers` 是非空数组）→ 直接用。
		 *
		 * @param {object|null} draft 当前草稿。
		 * @returns {{speakers: Array<object>, mode: "list"|"legacy"|"empty", legacyDid: string}}
		 */
		function normalizeSpeakers(draft) {
			const raw = draft && Array.isArray(draft.speakers) ? draft.speakers : null;
			const legacyDid = draft && draft.did ? String(draft.did) : "";

			// 情况 3：后端已经给了 speakers[]，照单全收。
			if (raw && raw.length > 0) {
				return {
					mode: "list",
					legacyDid,
					speakers: raw.map((item, index) => ({
						did: String(item?.did ?? ""),
						...
						agentPreset: item?.agentPreset ?? null,
						provider: item?.provider ?? null,
						modelOverride: item?.model != null && item?.provider != null ? item.model : null,
			...
			// 情况 1 / 2：只有单 did（老配置，或后端还没上 speakers）→ 合成一台。
			if (legacyDid) {
				return {
					mode: "legacy",
					legacyDid,
					speakers: [
						{
							did: legacyDid,
							name: "",
							model: draft && draft.deviceModel ? String(draft.deviceModel) : "",
							enabled: draft && draft.enabled === undefined ? true : Boolean(draft && draft.enabled),
							workspace: null,
							agentPreset: null,
							provider: null,
							modelOverride: null,
			...
			// 情况 0：完全没配 → 空列表（UI 显示「尚未添加音箱」+ 添加按钮）。
			return { mode: "empty", legacyDid: "", speakers: [] };
		}

		/**
		 * 设备状态点语义（设计 §6.2 的 `data-tone`）。
		 *
		 * 优先级：停用 > 出错 > 连接中 > 在线。
		 * 之所以「停用」最高：一台被用户主动关掉的音箱显示成红色「错误」
		 * 会让人以为坏了，语义上它是「有意为之」。
		 *
		 * @param {object} runtime 该设备的运行时状态（可能为 undefined）。
		 * @param {object} device  该设备的配置项。
		 * @returns {{tone: string, text: string}}
		 */
		function speakerTone(runtime, device) {
			if (device && device.enabled === false) return { tone: "muted", text: "已停用" };
			if (!runtime) {
				// 后端还没提供 status.speakers[]（当前状态）——
				// 不谎报「在线」，如实说「状态未知」。
				return { tone: "muted", text: "状态待同步" };
			}
			if (runtime.phase === "error" || runtime.lastError) return { tone: "error", text: "连接失败" };
			if (runtime.phase === "degraded") return { tone: "warn", text: "部分降级" };
			if (runtime.connected === false) return { tone: "error", text: "未连接" };
			if (runtime.connected === true) return { tone: "ok", text: runtime.online === false ? "已连接（离线）" : "在线" };
		 ...
		 * 取某台设备的运行时状态。
		 *
		 * ⚠️ 降级：后端还没有 `status.speakers[]` 时，用**账号级**状态
		 * 给唯一那台设备兜底（当前实现只有一台，语义上是对的）；多台时
		 * 绝不共享账号级状态（会把 A 的在线状态安到 B 头上）。
		 *
		 * @param {object} status   xiaoai.status 快照。
		 * @param {object} device   设备配置项。
		 * @param {number} total    设备总数。
		 */
		function speakerRuntime(status, device, total) {
			const list = status && Array.isArray(status.speakers) ? status.speakers : null;
			if (list) return list.find((item) => String(item?.did) === String(device.did)) || null;
			if (total === 1 && status && status.speaker) {
				// 单设备：账号级状态就是它的状态，可以安全投影。
		 ...
		 * 设备卡片。
		 *
		 * ⚠️ 这是**纯函数**（不含 Hook），因此可以直接调用 —— 但仍统一用
		 * `h(DeviceCard, props)` 的形式渲染：一旦将来给它加 Hook，
		 * 直接调用就会重演 React #310。现在保持规则一致，省得后人踩。
		 */
		function DeviceCard(options) {
			const device = options.device;
			const runtime = options.runtime;
			const tone = speakerTone(runtime, device);
			const expanded = Boolean(options.expanded);
			const name = device.name || (device.did ? "音箱 " + device.did : "未命名音箱");
			const overrides = [];

			// 「已覆盖」清单 —— 只在真正有覆盖值时显示（sparse 语义）。
			if (device.workspace) overrides.push("工作区 = " + device.workspace);
			if (device.agentPreset) overrides.push("Agent 预设 = " + device.agentPreset);
			if (device.provider && device.modelOverride) {
				overrides.push("模型 = " + device.provider + " / " + device.modelOverride);
			}

			const head = h(
				"div",
				{ className: "xiaoai-device-head", key: "head" },
				h("span", {
					className: "xiaoai-device-dot",
					"data-tone": tone.tone,
					title: tone.text,
					"aria-hidden": "true"
				}),
				h(
					"span",
					{ className: "xiaoai-device-name" },
					name,
					h("span", { className: "xiaoai-device-tone-text" }, tone.text)
				),
				device.model ? h("span", { className: "xiaoai-device-model" }, device.model) : null,
				h(
					"span",
					{ className: "xiaoai-device-actions" },
					h(
						"button",
						{
							type: "button",
							className: "xiaoai-device-btn",
							disabled: Boolean(options.disabled),
							title: device.enabled === false ? "启用这台音箱" : "停用这台音箱",
							"aria-label": device.enabled === false ? "启用" : "停用",
							onClick: () => options.onToggleEnabled(device)
						},
					...
					h(
						"button",
						{
							...
							title: "该设备的独立配置",
							"aria-label": "配置",
							onClick: () => options.onToggleExpand(device)
						},
						"⚙"
					),
					h(
						...
						{
							type: "button",
							className: "xiaoai-device-btn xiaoai-device-btn-danger",
							disabled: Boolean(options.disabled),
							title: "移除这台音箱",
							"aria-label": "移除",
							onClick: () => options.onRemove(device)
						},
						"🗑"
					)
				)
			...
			const meta = h(
				"div",
				{ className: "xiaoai-device-meta", key: "meta" },
				h("code", { className: "xiaoai-device-did" }, "did: " + (device.did || "（未填写）")),
				// 后端尚未提供设备列表时如实说明，不要让用户以为功能坏了。
				device.synthetic
					? h(
							"span",
							{ className: "xiaoai-device-badge" },
							"单设备模式（此版本后端尚未提供设备列表）"
						)
					: null
			);
			...
			const body = [
				...
				overrides.length > 0
					? h(
							"p",
							{ className: "xiaoai-device-override-summary", key: "ov" },
							"▸ 已覆盖：" + overrides.join("；")
						)
					: h(
							"p",
							{ className: "xiaoai-device-override-summary xiaoai-muted", key: "ov" },
							"▸ 使用全局配置（工作区 / 模型 / 预设）"
						)
			];

			// ⚙ 内联展开：覆盖项配置。未覆盖的字段显示「继承全局: <实际值>」。
			if (expanded) {
				body.push(
					h(
						"div",
						{ className: "xiaoai-device-config", key: "cfg" },
						FieldRow({
							key: "d-name",
							id: "dev-name-" + device.did,
							label: "名称",
							control: TextInput({
								value: device.name || "",
								onChange: (value) => options.onFieldChange(device, "name", value),
								placeholder: "给这台音箱起个名字"
							})
						}),
						FieldRow({
							key: "d-ws",
							id: "dev-ws-" + device.did,
							label: "工作区",
							hint: "留空 = 继承全局",
							control: TextInput({
								value: device.workspace || "",
								onChange: (value) => options.onFieldChange(device, "workspace", value || null),
								placeholder: options.inheritText("workspace")
							})
						}),
						FieldRow({
							key: "d-preset",
							id: "dev-preset-" + device.did,
							label: "Agent 预设",
							hint: "留空 = 继承全局",
							control: TextInput({
								value: device.agentPreset || "",
								onChange: (value) => options.onFieldChange(device, "agentPreset", value || null),
								placeholder: options.inheritText("agentPreset")
							})
						}),
						FieldRow({
							key: "d-provider",
							id: "dev-provider-" + device.did,
							label: "模型 provider",
							hint: "与 model 必须成对填写；留空 = 继承全局",
							control: TextInput({
								value: device.provider || "",
								onChange: (value) => options.onFieldChange(device, "provider", value || null),
								placeholder: options.inheritText("provider")
							})
						}),
						FieldRow({
							key: "d-model",
							id: "dev-model-" + device.did,
							label: "模型 model",
							hint: "与 provider 必须成对填写；留空 = 继承全局",
							control: TextInput({
								value: device.modelOverride || "",
								onChange: (value) => options.onFieldChange(device, "modelOverride", value || null),
								placeholder: options.inheritText("model")
							})
						}),
						overrides.length > 0
							? h(
									"div",
									{ className: "xiaoai-actions" },
									Button("恢复为全局默认", () => options.onResetOverrides(device), {
										disabled: Boolean(options.disabled)
									})
								)
							: null
			...
			return h(
				"div",
				{
					className:
						"xiaoai-device-card" +
						(device.enabled === false ? " xiaoai-device-disabled" : "") +
						(expanded ? " xiaoai-device-expanded" : ""),
					"data-tone": tone.tone
				},
				body
			);
		...
		/** ② 音箱设备的组内容（设备列表 + 添加/扫描入口）。 */
		function renderSpeakerGroup(state) {
			const info = state.speakerInfo;
			const children = [];

			// 顶部摘要：N 台 · M 在线
			if (info.speakers.length > 0) {
				const online = info.speakers.filter((d) => {
					const rt = state.runtimeFor(d);
					return rt && rt.connected !== false && d.enabled !== false;
				}).length;
				children.push(
					h(
						"p",
						{ className: "xiaoai-device-summary", key: "sum" },
						"共 " + info.speakers.length + " 台 · " + online + " 台在线"
					)
				);
			}
			...
			if (info.mode !== "list") {
				children.push(
					h(
						"p",
						{ className: "xiaoai-muted xiaoai-device-note", key: "mode-note" },
						info.mode === "legacy"
							? "当前为单设备配置。后端尚未提供设备列表时，界面回退为单设备模式，配置照常可用。"
							: "还没有配置音箱。点下方「添加音箱」扫描账号下的设备，或直接用接入向导。"
					)
				);
			}
			...
			if (info.speakers.length === 0) {
				children.push(
					h("p", { className: "xiaoai-muted", key: "empty" }, "尚未添加任何音箱。")
				);
			} else {
				children.push(
					h(
						"div",
						{ className: "xiaoai-device-list", key: "list" },
						info.speakers.map((device, index) =>
							// ⚠️ h(DeviceCard, …) 而不是 DeviceCard(…)：
							// 保持与「含 Hook 的组件」一致的渲染方式，
							// 将来给卡片加 Hook 时不会突然白屏（React #310）。
							h(DeviceCard, {
								key: device.did || "dev-" + index,
								device,
								runtime: state.runtimeFor(device),
								expanded: state.expandedDid === device.did,
								disabled: state.busy,
								inheritText: state.inheritText,
								onToggleEnabled: state.onToggleDeviceEnabled,
								onToggleExpand: state.onToggleDeviceExpand,
								onRemove: state.onRemoveDevice,
								onFieldChange: state.onDeviceFieldChange,
								onResetOverrides: state.onResetDeviceOverrides
							})
						)
					)
				);
			...
			if (state.discovered !== null) {
				children.push(
					h(
						"div",
						{ className: "xiaoai-device-picker", key: "picker" },
						SectionTitle("选择要添加的音箱"),
						state.discoveredLoading
							? h("p", { className: "xiaoai-muted" }, "正在扫描账号下的设备…")
							: null,
						state.discoverError
							? h("p", { className: "xiaoai-error", role: "alert" }, state.discoverError)
							: null,
						!state.discoveredLoading && state.discovered && state.discovered.length === 0
							? h("p", { className: "xiaoai-muted" }, "账号下没有找到可用的音箱。")
							: null,
						(state.discovered || []).map((cand) => {
							const already = info.speakers.some((d) => String(d.did) === String(cand.did));
							return h(
								"div",
								{ className: "xiaoai-device-candidate", key: cand.did },
								h(
									"span",
									{ className: "xiaoai-device-candidate-name" },
									cand.name || cand.did,
									h(
										"span",
										{ className: "xiaoai-muted" },
										"　" + (cand.model || "未知型号") + " · " + (cand.online ? "在线" : "离线")
									)
								),
								already
									? h("span", { className: "xiaoai-muted" }, "已添加")
									: Button("添加", () => state.onAddDevice(cand), {
											disabled: state.busy
										})
							);
						}),
						...
						h(
							"div",
							{ className: "xiaoai-actions" },
							Button("关闭", state.onClosePicker, { disabled: state.busy })
						)
					)
				);
			}
			...
			children.push(
				h(
					"div",
					{ className: "xiaoai-actions xiaoai-device-toolbar", key: "bar" },
					Button("+ 添加音箱", state.onOpenPicker, { disabled: state.busy }),
					Button("🔄 重新扫描账号设备", state.onRescan, { disabled: state.busy })
				)
			);

			return h("div", { className: "xiaoai-speakers-group" }, children);
		}

		/**
		 * 状态区里的「设备」小节（设计 §6.4）。
		 *
		 * 与设备卡片不同：这里**只读**，展示每台的连接态 / AI 模式 / 最近活动。
		 * 后端未提供 `status.speakers[]` 时，退回单设备投影 —— 不显示空表。
		 */
		function renderSpeakerStatusList(options) {
			const speakers = options.speakers || [];
			if (speakers.length === 0) return null;

			const rows = speakers.map((device, index) => {
				const runtime = options.runtimeFor(device);
				const tone = speakerTone(runtime, device);
				const name = device.name || device.did || "音箱 " + (index + 1);
				const bits = [];
				if (device.model) bits.push(device.model);
				if (runtime && runtime.aiMode) {
					const meta = aiModeMeta(runtime.aiMode);
					bits.push("AI 模式: " + runtime.aiMode);
					// aiModeMeta 的颜色也带上，和头部徽章语义一致。
					bits.push(meta.text);
				}
				if (runtime && runtime.lastHeard && runtime.lastHeard.at) {
					bits.push("最近: " + relativeTime(runtime.lastHeard.at));
				}
				return h(
					"div",
					{ className: "xiaoai-status-device", key: device.did || "s" + index },
					h("span", {
						className: "xiaoai-device-dot",
						"data-tone": tone.tone,
						title: tone.text,
					...
					h("span", { className: "xiaoai-status-device-name" }, name),
					h("span", { className: "xiaoai-status-device-tone" }, tone.text),
					bits.length > 0
						? h("span", { className: "xiaoai-muted xiaoai-status-device-bits" }, bits.join(" · "))
						: null
				);
			});
			...
			return h(
				"div",
				{ className: "xiaoai-status-devices" },
				SectionTitle("设备"),
				rows
			);
		}
		...
		const GROUP_FIELDS = {
			...
			// ⚠️ speakers 是「数组字段」，computeDirtyKeys 会逐项深比较 ——
			// 见下方 makeSpeakersComparable 的处理。
			devices: ["speakers"],
			access: ["enabled", "userId", "password"],
			behavior: [
				"aiModeEnabled",
				"pollIntervalMs",
				"maxReplyChars",
		...
		function renderSettingsForm(state) {
			...
			// 设备字段（did / deviceModel）已迁到「② 音箱设备」组。
			//
			// ⚠️ 但当**没有任何设备**时（既没 speakers 也没 did），这里仍渲染
			// 一个 did 输入框 —— 否则用户会陷入「想加设备但面板上没有入口」的死角
			// （首次接入的人只能走向导，而手工配置路径需要能直接填 DID）。
			const accessBody = [
				FieldRow({
					key: "enabled",
					id: "enabled",
				...
				state.speakerInfo.speakers.length === 0
					? FieldRow({
							key: "did",
							id: "did",
							label: "音箱 DID",
							hint: "设备 ID 或米家名称。也可以去「② 音箱设备」组点「添加音箱」自动扫描。",
							control: TextInput({
								value: draft.did,
								onChange: set("did"),
			...
			const behaviorBody = [
				...
				HelpTip({
					...
					text:
						"三类关键词的差别：\n" +
						"· 「进入 AI 模式」—— 说了之后进入连续对话，后面每句话都会交给 DSH，不用再喊触发词。\n" +
						"· 「退出 AI 模式」—— 说了就回到待命，普通话不再被处理。\n" +
						"· 「直接问」—— 只把这一句立刻交给 DSH，说完仍停留在原来的模式。\n\n" +
						"不设「退出」词也可以：静默超过「静默退出时长」会自动退出。"
				}),
				FieldRow({
					key: "wakeUpKeywords",
					id: "wakeUpKeywords",
			...
			const bodies = {
				session: sessionBody,
				// ⚠️ renderSpeakerGroup 是**模块级函数**（不是 state 上的方法），
				// 必须显式调用并把 state 传进去 —— 早期写成 state.renderSpeakerGroup()
				// 会抛 "state.renderSpeakerGroup is not a function"，
				// 整块面板渲染失败（浏览器实测抓到）。
				devices: renderSpeakerGroup(state),
				access: accessBody,
		...
		function XiaoaiSection(props) {
			...
			const rpc = props.rpc;
			const subscribeLoopbackRecovery = props.subscribeLoopbackRecovery;
			...
			/**
			 * 并发版本栅栏（见 useSettingsFence 的说明）。
			 *
			 * ⚠️ 必须在所有 early return 之前调用 —— 它内含 useRef/useMemo。
			 */
			const fence = useSettingsFence();
			...
			/** 回环地址恢复建议（rpc() 推上来）；null 表示无。 */
			const [recovery, setRecovery] = React.useState(null);

			// ── 多音箱设备卡片状态 ──
			/** 当前展开 ⚙ 配置的设备 did（同时只展开一台，避免面板过长）。 */
			const [expandedDid, setExpandedDid] = React.useState(null);
			/** 扫描结果；null = 未打开选择器。 */
			const [discovered, setDiscovered] = React.useState(null);
			const [discoveredLoading, setDiscoveredLoading] = React.useState(false);
			const [discoverError, setDiscoverError] = React.useState(null);

			// ── 焦点管理：弹层关闭后把焦点还给触发它的按钮 ──
			//
			// 无障碍要求：打开弹层 / 展开大块内容后关闭，焦点若不还回去
			// 就会掉到 <body>，键盘用户得从头 Tab 一遍。
			// 两个触发器：向导入口、日志展开。
			const wizardTriggerRef = React.useRef(null);
			const logsTriggerRef = React.useRef(null);

			/** 折叠/展开日志；收起时把焦点还给「查看日志」。 */
			const onToggleLogs = React.useCallback(() => {
				setLogsOpen((open) => {
					if (open) scheduleFocus(logsTriggerRef);
					return !open;
				});
			}, []);
			...
			// ── 回环恢复订阅 ──
			React.useEffect(() => {
				if (typeof subscribeLoopbackRecovery !== "function") return undefined;
				return subscribeLoopbackRecovery(setRecovery);
			}, [subscribeLoopbackRecovery]);
			...
			/**
			 * 关闭向导并把焦点还给触发它的「重新接入 / 更换音箱」按钮。
			 *
			 * ⚠️ 定义在 showWizard 之后、使用它的地方之前 —— 免得
			 * 依赖「回调只在挂载后才执行」这一隐式时序（TDZ 风险）。
			 */
			const closeWizard = React.useCallback(() => {
				setShowWizard(false);
				scheduleFocus(wizardTriggerRef);
			}, []);
			 ...
			 *
			 * 并发语义（本次新增）：服务端的 revision 是**乐观锁**，
			 * 另一个标签页 / 手机改过之后，我们手里这份 revision 就过期了，
			 * 保存会被拒。以前这里只显示「保存失败：<原始 message>」——
			 * 用户看不懂，也不知道该怎么办（验收报告 P2 记录的就是这条）。
			 * 现在识别出冲突后给出可操作的提示。
			 */
			const onSave = React.useCallback(async () => {
				...
				setBusyAction("save");
				setNotice(null);
				// 发起写：版本递增，任何在途的 settings.get 响应就此过期，
				// 不会在保存成功后把旧值又刷回界面。
				...
				try {
					...
					if (!fence.canCommitMutation(fenceVersion)) return;
					setRevision(result ? result.revision : undefined);
					const next = draftFromValues(result && result.values);
					setDraft(next);
					setBaseline(next);
					...
					if (isSettingsConflict(error)) {
						// 冲突是**预期内**的并发结果，不是崩溃：说清楚发生了什么、
						// 该怎么办，并直接给一个「重新加载设置」按钮 ——
						// 只丢一句「请刷新后重试」等于把活推回给用户。
						// ⚠️ 服务端把 expected/actual 放在 `details` 里
						// （实测 xiaoai/settings.update 的 RemoteError 形状），
						// 不是 `data`。
						setNotice({
							kind: "conflict",
							text:
								"配置已被其他地方修改（可能是另一个标签页或手机），" +
								"你的这次保存没有生效。请先重新加载最新配置，再重试。",
							action: "reload"
						});
					} else {
			...
			}, [call, draft, revision, draftFromValues, fence]);
			...
			/**
			 * 冲突后重新加载设置 —— 把最新 revision 和服务端真值拉回来。
			 * 用户在「配置已被其他地方修改」提示上点这个按钮即走此路径。
			 */
			const onReloadSettings = React.useCallback(async () => {
				setBusyAction("reload");
				setNotice(null);
				try {
					await loadSettings();
					setNotice({ kind: "ok", text: "已重新加载最新配置，可以再次修改并保存。" });
				} finally {
					setBusyAction(null);
				}
			}, [loadSettings]);
			...
			//#region 多音箱设备操作
			//
			// 所有设备编辑都写进 `draft.speakers`（数组），走既有的
			// 「改草稿 → 保存」链路 —— 不引入独立的保存路径，
			// 这样脏点、撤销修改、revision 乐观锁全部自动沿用。

			/** 当前设备列表视图（含降级信息）。 */
			const speakerInfo = React.useMemo(() => normalizeSpeakers(draft), [draft]);

			/** 取某台设备的运行时状态（后端未就绪时返回 null）。 */
			const runtimeFor = React.useCallback(
				(device) => speakerRuntime(status, device, speakerInfo.speakers.length),
				[status, speakerInfo.speakers.length]
			);

			/**
			 * 「继承全局: <实际值>」文案 —— 驱动覆盖态输入框的 placeholder。
			 * 用户一眼就能看出「留空会得到什么」，不用来回翻上面的组。
			 */
			const inheritText = React.useCallback(
				(field) => {
					const value = draft ? draft[field] : "";
					const shown = value === undefined || value === null || value === "" ? "（未设置）" : String(value);
					return "继承全局: " + shown;
				},
				[draft]
			);
			...
			/**
			 * 把新数组写回 draft.speakers（并同步老投影字段）。
			 *
			 * ⚠️ 这里是**向后兼容最容易翻车的地方**（浏览器实测踩过）：
			 * 老配置只有 `did`、没有 `speakers[]`，UI 会合成一台 `synthetic`
			 * 设备来显示。用户在卡片上改了一个字段 → `toStored()` 把 synthetic
			 * 项过滤掉（因为它不该被写进 `speakers[]`）→ 得到**空数组** →
			 * 老投影 `did` 被清空 → `notConfigured` 变成 true →
			 * 整个面板突然跳回「接入向导」，用户以为配置丢了。
			 *
			 * 因此：
			 *   · 有真实设备时，`did` 投影跟随第一台（启用优先）；
			 *   · **没有真实设备时，绝不清空 did**（保持老配置原样）。
			 */
			const writeSpeakers = React.useCallback((next, syncLegacy) => {
				setDraft((previous) => {
					if (previous === null) return previous;
					const out = { ...previous, speakers: next };
					if (syncLegacy && next.length > 0) {
						const first = next.find((d) => d.enabled !== false) || next[0];
						out.did = first ? String(first.did || "") : previous.did;
						if (first && first.model) out.deviceModel = String(first.model);
					}
					// next 为空时不碰 did / deviceModel —— 老配置必须原样保留。
					return out;
				});
			}, []);
			 ...
			 * 把视图项还原成「可保存的形状」。
			 *
			 * ⚠️ 关于 `synthetic`（老配置合成的单设备）：
			 * 早期实现直接 `.filter(d => !d.synthetic)` 把它丢掉，
			 * 导致「老配置下改任何设备字段都不生效」（改完输入框被 React 回滚，
			 * 脏点也不亮 —— 浏览器实测抓到）。
			 *
			 * 正确语义：**一旦用户编辑了这台设备，就应该把它升级成真实的
			 * speakers[] 条目**（写入时带上 did，不丢设备），而不是丢弃。
			 * 这同时也是老配置 → 新结构的自然迁移点。
			 */
			const toStored = React.useCallback((list) => {
				return list
					.map((d) => ({
						did: d.did,
						name: d.name || "",
						// 型号：覆盖态用设备自己的 modelOverride，否则用设备型号。
						model: d.modelOverride || d.model || "",
						enabled: d.enabled !== false,
						workspace: d.workspace ?? null,
						agentPreset: d.agentPreset ?? null,
						provider: d.provider ?? null
					}))
					.filter((d) => d.did !== "");
			}, []);

			const onToggleDeviceEnabled = React.useCallback(
				(device) => {
					const next = speakerInfo.speakers.map((d) =>
						d.did === device.did ? { ...d, enabled: d.enabled === false } : d
					);
					writeSpeakers(toStored(next), true);
				},
				[speakerInfo.speakers, toStored, writeSpeakers]
			);

			const onToggleDeviceExpand = React.useCallback((device) => {
				setExpandedDid((previous) => (previous === device.did ? null : device.did));
			}, []);

			const onDeviceFieldChange = React.useCallback(
				(device, field, value) => {
					const next = speakerInfo.speakers.map((d) =>
						d.did === device.did ? { ...d, [field]: value } : d
					);
					// 设备字段改动同时同步老投影（did / deviceModel 是单值字段）。
					writeSpeakers(toStored(next), true);
				},
				[speakerInfo.speakers, toStored, writeSpeakers]
			);

			const onResetDeviceOverrides = React.useCallback(
				(device) => {
					const next = speakerInfo.speakers.map((d) =>
						d.did === device.did
							? { ...d, workspace: null, agentPreset: null, provider: null, modelOverride: null }
							: d
					);
					writeSpeakers(toStored(next), true);
				},
			 ...
			 * 移除设备。
			 *
			 * 二次确认的文案刻意具体（设计 §6.2 点名要求）：说清「会话不再复用
			 * 但历史仍保留」，比笼统的「确定删除吗？」有用得多。
			 */
			const onRemoveDevice = React.useCallback(
				(device) => {
					const name = device.name || device.did || "这台音箱";
					const ok =
						typeof window === "undefined" ||
						window.confirm(
							"确定移除「" +
					...
					if (!ok) return;
					const next = speakerInfo.speakers.filter((d) => d.did !== device.did);
					writeSpeakers(toStored(next), true);
				},
				[speakerInfo.speakers, toStored, writeSpeakers]
			);
			...
			/**
			 * 扫描账号下的设备（复用已有的 discoverSpeakers —— 无需重新登录）。
			 *
			 * 无参调用：服务端用**已保存的凭据**去列设备，这正是「添加设备
			 * 不需要重新登录」的实现基础。
			 */
			const scanDevices = React.useCallback(async () => {
				...
				try {
					...
					if (result && result.needsAuth) {
						setDiscoverError(result.error || "需要在浏览器完成一次小米安全验证");
						setDiscovered([]);
						return;
					}
					if (!result || result.ok === false) {
						setDiscoverError((result && result.error) || "读取设备列表失败");
						setDiscovered([]);
						return;
					}
					setDiscovered(Array.isArray(result.speakers) ? result.speakers : []);
				} catch (error) {
					setDiscoverError("读取设备列表失败：" + describeError(error));
					setDiscovered([]);
				} finally {
			...
			const onAddDevice = React.useCallback(
				(candidate) => {
					const did = String(candidate.did || "");
					if (!did) return;
					if (speakerInfo.speakers.some((d) => String(d.did) === did)) return;
					const added = {
						did,
						...
						modelOverride: null,
						overrides: null,
						_index: speakerInfo.speakers.length,
						synthetic: false
					};
					// ⚠️ 首次从「降级单设备」升级到「列表」时，原有的合成项
					// 不能丢 —— 它是用户当前真正在用的那台。
					const base = speakerInfo.speakers.filter((d) => !d.synthetic || d.did !== added.did);
					const merged = base.some((d) => String(d.did) === did) ? base : [...base, added];
					writeSpeakers(toStored(merged), true);
					// 从候选列表里划掉刚添加的
					setDiscovered((previous) =>
						(previous || []).filter((c) => String(c.did) !== did)
					);
				},
				[speakerInfo.speakers, toStored, writeSpeakers]
			);
			...
			// ⚠️ 以下三个 Hook 必须在 `if (wizardActive) return` 【之前】执行。
			// 原因：wizardActive 会翻转（首渲染 draft=null → 走向导；数据到位后
			// → 走正常分支），若 Hook 在两个分支里数量不同，React 抛 #310
			// 「Rendered more hooks than during the previous render」，
			// 表现为整个设置面板白掉。实测踩过（面板内容区 xiaoai-class 元素数为 0）。
			// ── 脏检查（§6.4）──
			//
			// 数组字段不能用 !== 比（normalizeSettings 每次都建新数组），
			...
			const renderStatusGroup = React.useCallback(
				() =>
					h(
						...
						// ── 设备状态列表（设计 §6.4）──
						//
						// 后端提供 status.speakers[] 时逐台列出；否则退回
						// 单设备视图（用 status.speaker 投影）—— 保证老后端也正常。
						renderSpeakerStatusList({
						...
						renderLogs({
							...
							onToggleLogs,
							// 焦点管理：日志收起时把焦点还给这个按钮。
							toggleRef: logsTriggerRef
						})
					),
				[status, logsOpen, logs, logsLoading, logsError, onToggleLogs, speakerInfo.speakers, runtimeFor]
			);
				...
				return h(
					...
					!notConfigured
						? h(
								...
								// 用 closeWizard 而不是裸 setShowWizard(false)：
								// 关闭后焦点要回到「重新接入 / 更换音箱」按钮。
								Button("关闭向导", closeWizard)
							)
						: null
				);
			}
			...
			const children = [
				...
				// ── 回环地址恢复提示（最高优先级，置于所有其它提示之上）──
				//
				// 出现条件：rpc() 观测到 127.0.0.1 的本机请求校验失败
				// （`transport failure for /api/xiaoai/<m>: HTTP 403`）。
				// 这不是插件坏了，而是地址不对 —— 给一个能直接点的出口，
				// 比让用户对着「网络错误」发呆有用。
				recovery
					? h(
							"div",
							{ className: "xiaoai-recovery", role: "alert", key: "recovery" },
							h(
								"p",
								{ className: "xiaoai-recovery-text" },
								"当前地址（" +
									(typeof window !== "undefined" ? window.location.origin : "127.0.0.1") +
									"）与浏览器的本机请求校验不兼容，导致请求被拒绝。"
							),
							h(
								"div",
								{ className: "xiaoai-actions" },
								Button(
									"改用 localhost 重新打开",
									() => {
										// replace 而不是 assign：不要把这个已知有问题的地址
										// 留在浏览历史里，否则用户点「后退」又回到坏状态。
										if (typeof window !== "undefined" && window.location) {
											window.location.replace(recovery.url);
										}
									},
									{ primary: true, title: recovery.url }
								),
								Button("忽略", () => setRecovery(null))
							)
						)
					: null,
				notice
					? h(
							...
							notice.action === "reload"
								? h(
										...
										{
											type: "button",
											className: "xiaoai-button xiaoai-notice-action",
		 ...
		 * 并用错误边界兜住整棵子树。
		 *
		 * ⚠️ 这里**必须**用 h(PanelErrorBoundary, …) 包装而不是 try/catch ——
		 * React 的错误只能在渲染期间由边界组件捕获，普通 try/catch 抓不到
		 * 子组件的渲染异常（这正是白屏无法自救的原因）。
		 *
		 * ⚠️ `h(XiaoaiSection, props)` 而不是 `XiaoaiSection(props)`：
		 * XiaoaiSection 内含 Hook，必须当组件渲染；直接调用会把它的 Hook
		 * 挂到本包装函数的 fiber 上 —— 这就是 2026-09-21 那次 React #310 的成因。
		 */
		...
		function apply(ctx) {
			 ...
			 * 回环恢复的订阅槽。
			 *
			 * rpc() 在模块闭包里、UI 在组件里，两者不能直接通信；
			 * 用一个极小的「最后恢复建议 + 订阅者集合」把信号送上去。
			 * 之所以不放进 React state：rpc() 可能在组件挂载前就被调用
			 * （createStatusSource 首次 getSnapshot），那时还没有 setState 可调。
			 */
			let loopbackRecovery = null;
			const loopbackListeners = new Set();
			const publishLoopbackRecovery = (recovery) => {
				if (!recovery) return;
				// 已经提示过同一个目标就不重复推送，避免轮询刷屏。
				if (loopbackRecovery && loopbackRecovery.url === recovery.url) return;
				loopbackRecovery = recovery;
				for (const listener of [...loopbackListeners]) {
					try {
						listener(recovery);
					} catch (error) {
						console.error("xiaoai: loopback listener failed:", error);
					}
				}
			};
			const subscribeLoopbackRecovery = (listener) => {
				loopbackListeners.add(listener);
				// 立即回放一次：组件挂载晚于首次失败时也能看到提示。
				if (loopbackRecovery) listener(loopbackRecovery);
			...
			const rpc = async (method, args) => {
				...
				try {
					...
					// 服务端直接回 403：dsh-client-connection 会把这种情况
					// 包成 `transport failure for /api/xiaoai/<m>: HTTP 403`
					// （见 dsh-client-connection/lib/client.js:1131）。
					// 这里用同一签名走同一套判定，并且**直接抛恢复错误** ——
					// 不再往下 `res.json()`：403 的响应体通常是空的，硬解析
					// 只会抛出一个语义错误（"非 JSON 响应"），把真正的原因盖掉。
					if (!res.ok && res.status === 403) {
						const simulated = new Error(
							"transport failure for /api/" + endpoint + ": HTTP 403"
						);
						const recovery = createLoopbackRecovery(simulated);
						if (recovery) {
							publishLoopbackRecovery(recovery);
							const presented = new Error(LOOPBACK_RECOVERY_MESSAGE);
							presented.code = "loopback-recovery-required";
							presented.cause = simulated;
							presented.recoveryUrl = recovery.url;
							throw presented;
						}
					}
					if (!res.ok) {
						throw new Error("网关返回 HTTP " + res.status + "（" + endpoint + "）");
					}
					body = await res.json();
				} catch (error) {
					// fetch 阶段直接抛（CORS / PNA 拦截 / 连接被拒）——
					// 这正是回环地址校验最常见的表现。
					if (error && error.code === "loopback-recovery-required") throw error;
					const recovery = createLoopbackRecovery(error);
					if (recovery) {
						publishLoopbackRecovery(recovery);
						const presented = new Error(LOOPBACK_RECOVERY_MESSAGE);
						presented.code = "loopback-recovery-required";
					...
					if (error && error.name === "SyntaxError") {
						throw new Error("网关返回了非 JSON 响应（" + endpoint + "）");
					}
					throw error;
				}
				...
				if (result.ok === false) {
					...
					// ⚠️ 必须保留 code：上游 fail() 的失败分类
					// （xiaoai/settings-conflict 等）全靠它，丢了码就退化成
					// 「未知错误」，并发冲突提示也就无从触发。
					const failure = new Error(err.message || err.code || "RPC 调用失败");
					if (err.code) failure.code = err.code;
					// 服务端的结构化补充（如冲突的 expected/actual）挂在 `details`，
					// 保留下来供 UI/排查使用。
					if (err.details !== undefined) failure.details = err.details;
					if (err.data !== undefined) failure.data = err.data;
					throw failure;
				}
				return result.value;
			};
				...
				ctx.slots.register(
					{
						...
						inject: () => ({
							...
							rpc,
							// 回环恢复订阅（rpc() 里检测到 127.0.0.1 的 403 类
							// 传输错误时推上来，组件据此渲染「改用 localhost」按钮）。
							subscribeLoopbackRecovery
						})
					},
					XiaoaiSectionBound
