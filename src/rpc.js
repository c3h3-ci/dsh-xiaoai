...
const RESTART_KEYS = Object.freeze(["enabled", "userId", "password", "did"]);
...
export function createXiaoaiControllerClass(protocol, runtime, deps) {
  ...
  class XiaoaiController extends TypertRemoteService {
    ...
    /**
     * `xiaoai.test({ text })` → `{ ok, reply }`；走完整链路但不播报。
     *
    ...
    async speak(args) {
      ...
      try {
        ...
        return await runtime.speak(text);
      } catch (err) {
        throw fail(err, endpoint);
      }
    }
    ...
    async onboardingApply(args) {
      ...
      try {
        ...
        // 写设置（userId/did）—— 这是 runtime 实际读取的字段。
        // 密码不写：token 已经在 store 里了，写明文密码进设置文件
        // 只会多一份泄漏面，且 sanitizePatch 对空串的语义是"未修改"。
        const patch = { userId: written.userId, did: finalDid };
